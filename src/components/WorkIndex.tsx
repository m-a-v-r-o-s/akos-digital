...
const L = {
  en: {
    back: "Back to home",
    eyebrow: "Work",
    title: "Live client sites",
    intro:
      "Three businesses running on sites I built and still support. Each case study says what the problem actually was and what was done about it. There are no invented conversion figures here: every claim is something you can check by opening the site.",
  ...
  el: {
    ...
    eyebrow: "Έργα",
    title: "Ζωντανά sites πελατών",
    intro:
      "Τρεις επιχειρήσεις που λειτουργούν πάνω σε sites που έχτισα και συνεχίζω να υποστηρίζω. Κάθε μελέτη περίπτωσης λέει ποιο ήταν πραγματικά το πρόβλημα και τι έγινε γι' αυτό. Δεν υπάρχουν επινοημένα ποσοστά μετατροπής εδώ: κάθε ισχυρισμός είναι κάτι που μπορείς να ελέγξεις ανοίγοντας το site.",
                ...
                {caseStudies.map((c) => (
                  <li key={c.slug}>
                    <Link href={`/${lang}/work/${c.slug}`} className="section-card group block">
                      <p className="font-mono text-xs tracking-[0.2em] uppercase text-stone mb-2">
                        {c.client}
                      </p>
